...
-- CREATE TRIGGER update_accumulated_cash IF NOT EXISTS
--   AFTER INSERT ON HISTORY
--       BEGIN
--         UPDATE USERS SET ACCUMULATED_CASH = ACCUMULATED_CASH + NEW.QUANTITY WHERE (USERS.UUID = (SELECT user.UUID FROM USERS as user, TRANSACTIONS as trans WHERE (user.UUID = trans.USER_ID AND NEW.TRANSACTION_ID = trans.ID)));
--           END;
