package feup.mieic.cmov.acme.qrcodes;
...
public class QRTag extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Thread t = new Thread(new Runnable() {
            @Override
            public void run() {              // do the creation in a new thread to avoid ANR Exception
                final Bitmap bitmap;
                try {
                    bitmap = QRTag.this.encodeAsBitmap(qr_content);
                    QRTag.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {                  // runOnUiThread method used to do UI task in main thread.
